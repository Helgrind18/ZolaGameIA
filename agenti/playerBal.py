...
# MEMOIZZAZIONE: Cache globale per velocizzare l'albero
EVAL_CACHE = {}
...
def evaluate_state(game, state, root_player):
    # 1. OTTIMIZZAZIONE CACHE: Trasformiamo la matrice in una tupla di tuple (molto più veloce da hashare rispetto a una stringa)
    ...
    # 2. PESI DINAMICI: Riduciamo l'importanza dei bordi se ci sono pochi pezzi (Endgame)
    total_pieces = my_pieces + opp_pieces
    current_border_multiplier = BORDER_MULTIPLIER if total_pieces > 10 else 2
    ...
    for r in range(state.size):
        for c in range(state.size):
            ...
            if piece is None:
                continue
            pos_value = levels[r][c] * current_border_multiplier
    ...
    # 3. BIVIO STRATEGICO
    if root_player == "Red":
        # Il Rosso gioca la strategia aggressiva matematica originale
        ...
        # Fattore Caos per disorientare i bot deterministici
