...
def _fallback_no_moves_winner(board, player_to_move):
    red = board.count(RED)
    blue = board.count(BLUE)
    if red > blue:
        return RED
...
def _winner(board, player_to_move, n, levels):
    # Compatibilità interna: terminale vero quasi sempre per materiale.
    # Il caso teorico senza mosse per entrambi viene gestito solo quando serve.
    return _material_winner(board)
...
def _evaluate(board, to_move, root, n, levels):
    ...
    if not op_moves and my_moves:
        score += W_PASS_ADVANTAGE
    if not my_moves and op_moves:
        score -= W_PASS_ADVANTAGE
...
def _alphabeta(board, to_move, depth, alpha, beta, root, n, levels, deadline, ply=0):
    ...
    if not moves:
        if not _has_moves(board, _opp(to_move), n, levels):
            w2 = _fallback_no_moves_winner(board, to_move)
            if w2 == root:
...
def playerStrategy(game, state, timeout=3):
    ...
    safe_margin = min(SAFETY_MARGIN, max(0.03, float(timeout) * 0.08))
    deadline = _now() + max(0.02, float(timeout) - safe_margin)
    ...
    for depth in range(1, MAX_DEPTH+1):
        ...
        if remaining <= max(0.06, last_depth_time * 3.0):
            break
        depth_start = _now()
        try:
            sc, cand = _alphabeta(board, to_move, depth, -INF, INF, root, n, levels, deadline, 0)
            last_depth_time = _now() - depth_start
            if cand in moves:
