"""
playerHelgrindV21.py
Versione V2 ritoccata in modo conservativo.

Obiettivo: non toccare la struttura vincente di V2, ma migliorare soprattutto
il caso difficile: giocare da Blu contro playerStrategyImplPasqualeOGRAW.

Cambiamenti rispetto a V2:
- stessi tempi e stessa ricerca alpha-beta compatta;
- pesi dinamici leggermente più difensivi quando root == Blue;
- minacce pesate anche in base al livello del pezzo;
- piccolo bonus/penalità per posizioni in cui un giocatore ha pochissime mosse;
- ordinamento mosse appena più prudente sulle catture che atterrano in case molto centrali.
"""
...
# Ritocchi piccoli: non devono cambiare il carattere di V2.
W_LOW_MOBILITY_TRAP = 650      # bonus se l'avversario ha pochissime mosse, penalità se siamo noi
W_PASS_PRESSURE = 1800         # bonus/penalità se uno dei due non ha mosse
...
def _move_points(move, levels):
    fr, to, cap = move
    if cap:
        # V2 ordinava molto bene le catture. Qui le teniamo prioritarie,
        # ma non vogliamo spingere troppo in basso/centro se esistono catture simili più sicure.
        central_risk = max(0, 4 - levels[to])
        return 100000 + 540*levels[fr] - 55*levels[to] - 90*central_risk*central_risk + (fr*11 + to*7)
...
def _weights_for(root):
    """Pesi dinamici ma conservativi.

    Il Rosso ha un vantaggio naturale perché parte per primo. Quando siamo Blu
    alziamo un po' sicurezza/minacce/mobilità e abbassiamo leggermente la fame
    di bordo, perché contro Pasquale da Rosso perdere pezzi tatticamente costa più
    del guadagno posizionale.
    """
    if root == BLUE:
        return {
            "material": W_MATERIAL,
    ...
    return {
        "material": W_MATERIAL,
        "capture": W_CAPTURE_MOBILITY,
        "mobility": W_MOBILITY,
        "outer": W_OUTER_LEVEL,
        "center": W_CENTER_DANGER,
        "threat": W_THREATENED,
        "turn": W_TURN,
    }
...
def _evaluate(board, to_move, root, n, levels):
    ...
    weights = _weights_for(root)

    my_p = board.count(root)
    op_p = board.count(opponent)
    total = my_p + op_p
    material_weight = weights["material"] + max(0, 20-total)*420
    ...
    score += weights["mobility"] * (len(my_moves) - len(op_moves))
    score += weights["capture"] * (len(my_caps) - len(op_caps))

    # Piccolo controllo anti-pass / anti-blocco.
    # Non deve dominare il materiale: serve solo a preferire posizioni dove il rivale
    # rimane con poche scelte, senza rendere l'agente passivo come V3.
    if len(op_moves) == 0:
        score += W_PASS_PRESSURE
    elif len(op_moves) <= 2:
        score += W_LOW_MOBILITY_TRAP * (3 - len(op_moves))

    if len(my_moves) == 0:
        score -= W_PASS_PRESSURE
    elif len(my_moves) <= 2:
        score -= W_LOW_MOBILITY_TRAP * (3 - len(my_moves))

    my_targets = _targets(my_caps)
    op_targets = _targets(op_caps)
    my_thr = op_thr = 0
    my_thr_value = op_thr_value = 0
    outer = 0
    center = 0
    for i, piece in enumerate(board):
        ...
        if piece == root:
            ...
            if i in op_targets:
                my_thr += 1
                # Perdere un pezzo esterno è grave: di solito è materiale + posizione.
                my_thr_value += 2 + lvl
        else:
            outer -= pos
            center += central*central
    ...
    score += weights["outer"] * outer + weights["center"] * center
    score += weights["threat"] * (op_thr - my_thr)
    score += 170 * (op_thr_value - my_thr_value)
    score += weights["turn"] if to_move == root else -weights["turn"]
