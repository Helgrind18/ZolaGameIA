...
Obiettivo: migliorare il punto debole contro strategie molto orientate a bordo/mobilità
senza specializzarsi troppo e senza perdere la robustezza generale di V2.

Cambiamenti rispetto a V2:
- stessi tempi, stessa ricerca alpha-beta e stessa rappresentazione interna compatta;
- nessun cambio drastico dei pesi quando gioca Blu;
- aggiunta di una piccola valutazione del valore delle minacce in base al livello;
- piccolo bonus/penalità per posizioni con mobilità estremamente bassa.
"""
...
# Ritocchi conservativi: molto più piccoli di V21.
# Servono a correggere qualche posizione anti-Pasquale senza trasformare V2
# in un agente troppo difensivo/specializzato.
W_THREAT_LEVEL_VALUE = 70
W_LOW_MOBILITY = 240
W_NO_MOVE_PRESSURE = 650
...
def _evaluate(board, to_move, root, n, levels):
    ...
    # Micro-bonus/penalità per trappole di mobilità.
    # Valori volutamente bassi: il materiale resta molto più importante.
    if len(op_moves) == 0:
        score += W_NO_MOVE_PRESSURE
    elif len(op_moves) <= 2:
        score += W_LOW_MOBILITY * (3 - len(op_moves))

    if len(my_moves) == 0:
        score -= W_NO_MOVE_PRESSURE
    elif len(my_moves) <= 2:
        score -= W_LOW_MOBILITY * (3 - len(my_moves))
    ...
    score += W_THREAT_LEVEL_VALUE * (op_thr_value - my_thr_value)
