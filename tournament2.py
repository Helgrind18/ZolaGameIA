...
import importlib  # AGGIUNTO: Necessario per caricare le strategie
import time  # AGGIUNTO: Necessario per calcolare i tempi e i timeout
import csv  # AGGIUNTO: Necessario per salvare il file Excel/CSV
...
# Timestamp aggiunto per evitare che più tornei in contemporanea sovrascrivano lo stesso file
TIMESTAMP = int(time.time())
FILE_RISULTATI = f"statistiche_{FILE_STRATEGIA_ROSSO}_vs_{FILE_STRATEGIA_BLU}_NP_{NUMERO_PARTITE}_TM_{TIMEOUT_MOSSA}_{TIMESTAMP}.csv"
...
def main():
    ...
    for i in range(1, NUMERO_PARTITE + 1):
        ...
        print(f"Partita {i} conclusa. Vince: {winner}")
